...
struct Vector2D
{
  ...
  inline Vector2D  SafeNormal() const;
  ...
  inline Vector2D  GetReverse()const;
	...
	inline Vector2D SafeRotate(float rad) const
	{	
		Vector2D result;
		result = Vector2D(x,y);
		float tx = (result.x * cosf(rad)) - (result.y * sinf(rad));
		float ty = (result.x * sinf(rad)) + (result.y * cosf(rad));
		result.x = tx;
		result.y = ty;
	}
...
inline Vector2D Vector2D::SafeNormal() const
{
	Vector2D Result;
