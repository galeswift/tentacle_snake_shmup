// File:  VGPhutils.c // taken verbatim from philip rideout's bloom tutorial http://prideout.net/bloom/
...
void phCheckFBO()
{
    ...
    fprintf(stderr,"incomplete framebuffer object due to %s", enums[status]);
}
...
void phCheckError(const char *call)
{
    ...
    fprintf(stderr,"OpenGL %s in '%s'", enums[errcode], call);
}
